...
@inline function thick_pure_bdipole(tm::BendKick, bunch, bm1, L) 
  if isnothing(bunch.coords.q) && !(tm.radiation_damping_on || tm.radiation_fluctuations_on)
    ...
    radiation_params = ifelse(tm.radiation_damping_on, (q, mc2, E_ref), nothing)
    params = (radiation_params, tilde_m, beta_0, a, 0, w, w_inv, k0, SA[mm], SA[kn], SA[ks])
    if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
      photon_params = (get_backend(bunch.coords.v), q, mc2, E_ref, 0, 0, SA[mm], SA[kn], SA[ks])
    else
      photon_params = nothing
...
@inline function thick_bdipole(tm::BendKick, bunch, bm, L)
  ...
  E_ref = mc2/tilde_m/beta_0
  radiation_params = ifelse(tm.radiation_damping_on, (q, mc2, E_ref), nothing)
  params = (radiation_params, tilde_m, beta_0, a, 0, w, w_inv, k0, mm, kn, ks)
  if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
    photon_params = (get_backend(bunch.coords.v), q, mc2, E_ref, 0, 0, mm, kn, ks)
  else
    photon_params = nothing
...
@inline function thick_pure_bquadrupole(tm::Union{Yoshida,MatrixKick}, bunch, bm, L)
  ...
  E_ref = mc2/tilde_m/beta_0
  radiation_params = ifelse(tm.radiation_damping_on, (q, mc2, E_ref), nothing)
  params = (radiation_params, beta_0, gamsqr_0, tilde_m, gyromagnetic_anomaly(bunch.species), w, w_inv, k1, SA[mm], SA[kn], SA[ks])
  if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
    photon_params = (get_backend(bunch.coords.v), q, mc2, E_ref, 0, 0, SA[mm], SA[kn], SA[ks])
  else
    photon_params = nothing
...
@inline function thick_bquadrupole(tm::Union{Yoshida,MatrixKick}, bunch, bm, L)
  ...
  E_ref = mc2/tilde_m/beta_0
  radiation_params = ifelse(tm.radiation_damping_on, (q, mc2, E_ref), nothing)
  params = (radiation_params, beta_0, gamsqr_0, tilde_m, gyromagnetic_anomaly(bunch.species), w, w_inv, k1, mm, kn, ks)
  if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
    photon_params = (get_backend(bunch.coords.v), q, mc2, E_ref, 0, 0, mm, kn, ks)
  else
    photon_params = nothing
...
@inline function thick_bend_pure_bdipole(tm::Union{Yoshida,BendKick}, bunch, bendparams, bm1, L)
  if isnothing(bunch.coords.q) && !(tm.radiation_damping_on || tm.radiation_fluctuations_on)
    ...
    if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
      photon_params = (get_backend(bunch.coords.v), q, mc2, E_ref, g, tilt, SA[mm], SA[Kn0], SA[Ks0])
    else
      photon_params = nothing
    end
    return integration_launcher(BeamTracking.bkb_multipole!, params, photon_params, tm, edge_params, L)
...
@inline function thick_bend_bdipole(tm::Union{Yoshida,BendKick}, bunch, bendparams, bm, L)
  ...
  if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
    photon_params = (get_backend(bunch.coords.v), q, mc2, E_ref, g, tilt, mm, kn, ks)
  else
    photon_params = nothing
  end
  return integration_launcher(BeamTracking.bkb_multipole!, params, photon_params, tm, edge_params, L)
...
@inline function thick_pure_rf(tm::Union{Yoshida,DriftKick}, bunch, rfparams, beamlineparams, L)
  ...
  mc2 = massof(bunch.species)
  radiation_params = ifelse(tm.radiation_damping_on, (q, mc2, E_ref), nothing)
  params = (radiation_params, beta_0, gamsqr_0, tilde_m, E_ref, gyromagnetic_anomaly(bunch.species), omega, t_ref, E0_normalized, 0, Val{false}(), SA[0], SA[0], SA[0])
  if isprimitivetype(eltype(bunch.coords.v)) && tm.radiation_fluctuations_on
