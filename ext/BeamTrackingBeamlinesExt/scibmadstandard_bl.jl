<<<<<<< HEAD
@inline drift(tm::SciBmadStandard, bunch, L) = drift(Exact(), bunch, L) 
=======
# Note: for exact transformations, use Yoshida order 2 because higher orders won't increase accuracy
@inline drift(tm::SciBmadStandard, bunch, L) = drift(Yoshida(order=2), bunch, L) 
@inline pure_patch(tm::SciBmadStandard, bunch, patchparams, L) = pure_patch(Exact(), bunch, patchparams, L)
>>>>>>> eiad/implicit_2.0

@inline pure_rf(tm::SciBmadStandard, bunch, rfparams, beamlineparams, L)                = pure_rf(Yoshida(), bunch, rfparams, beamlineparams, L)                        
@inline pure_bsolenoid(tm::SciBmadStandard, bunch, bm0, L)                              = pure_bsolenoid(Yoshida(order=2), bunch, bm0, L)                             
@inline bsolenoid(tm::SciBmadStandard, bunch, bmultipoleparams, L)                      = bsolenoid(Yoshida(), bunch, bmultipoleparams, L)                     
@inline pure_bdipole(tm::SciBmadStandard, bunch, bm1, L)                                = pure_bdipole(Yoshida(order=2), bunch, bm1, L)                               
@inline bdipole(tm::SciBmadStandard, bunch, bmultipoleparams, L)                        = bdipole(Yoshida(), bunch, bmultipoleparams, L)                       
@inline pure_bquadrupole(tm::SciBmadStandard, bunch, bm2, L)                            = pure_bquadrupole(Yoshida(), bunch, bm2, L)                           
@inline bquadrupole(tm::SciBmadStandard, bunch, bmultipoleparams, L)                    = bquadrupole(Yoshida(), bunch, bmultipoleparams, L)                   
@inline pure_bmultipole(tm::SciBmadStandard, bunch, bmk, L)                             = pure_bmultipole(Yoshida(), bunch, bmk, L)                            
@inline bmultipole(tm::SciBmadStandard, bunch, bmultipoleparams, L)                     = bmultipole(Yoshida(), bunch, bmultipoleparams, L)                    
@inline bmultipole_rf(tm::SciBmadStandard, bunch, bmultipoleparams, rfparams, beamlineparams, L) = bmultipole_rf(Yoshida(), bunch, bmultipoleparams, rfparams, beamlineparams, L)
@inline bend_no_field(tm::SciBmadStandard, bunch, bendparams, L)                        = bend_no_field(Yoshida(), bunch, bendparams, L)                       
@inline bend_pure_bsolenoid(tm::SciBmadStandard, bunch, bendparams, bm0, L)             = bend_pure_bsolenoid(Yoshida(), bunch, bendparams, bm0, L)            
@inline bend_bsolenoid(tm::SciBmadStandard, bunch, bendparams, bmultipoleparams, L)     = bend_bsolenoid(Yoshida(), bunch, bendparams, bmultipoleparams, L)    
@inline bend_pure_bdipole(tm::SciBmadStandard, bunch, bendparams, bm1, L)               = bend_pure_bdipole(Yoshida(), bunch, bendparams, bm1, L)              
@inline bend_bdipole(tm::SciBmadStandard, bunch, bendparams, bmultipoleparams, L)       = bend_bdipole(Yoshida(), bunch, bendparams, bmultipoleparams, L)      
@inline bend_pure_bquadrupole(tm::SciBmadStandard, bunch, bendparams, bm2, L)           = bend_pure_bquadrupole(Yoshida(), bunch, bendparams, bm2, L)          
@inline bend_bquadrupole(tm::SciBmadStandard, bunch, bendparams, bmultipoleparams, L)   = bend_bquadrupole(Yoshida(), bunch, bendparams, bmultipoleparams, L)  
@inline bend_pure_bmultipole(tm::SciBmadStandard, bunch, bendparams, bmk, L)            = bend_pure_bmultipole(Yoshida(), bunch, bendparams, bmk, L)           
@inline bend_bmultipole(tm::SciBmadStandard, bunch, bendparams, bmultipoleparams, L)    = bend_bmultipole(Yoshida(), bunch, bendparams, bmultipoleparams, L)         
@inline implicit_in(tm::SciBmadStandard, bunch)                                         = implicit_in(Yoshida(), bunch)
@inline implicit_body(tm::SciBmadStandard, bunch, fpp, bp, L)                           = implicit_body(Yoshida(), bunch, fpp, bp, L)
@inline implicit_out(tm::SciBmadStandard, bunch)                                        = implicit_out(Yoshida(), bunch)