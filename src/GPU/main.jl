...
function main() 
  if CUDA.functional()
    @info "CUDA is functional"
  else
    @info "CUDA is unavailible"
  end
...
function test_ele()
  ...
  ld2 = 0.75;  # m
  ld3 = 2.00;  # m
  dr1 = MatrixKick_GPUext.Drift(L = ld1)
  dr2 = MatrixKick_GPUext.Drift(L = ld2) 
  dr3 = MatrixKick_GPUext.Drift(L = ld3)
  ...
  gr3 = 1.20;  # T/m
  gr4 = 3.50;  # T/m
  qf1 = MatrixKick_GPUext.Quadrupole(L = lq1, Bn1 = +gr1)
  qd1 = MatrixKick_GPUext.Quadrupole(L = lq1, Bn1 = -gr1)
  qf2 = MatrixKick_GPUext.Quadrupole(L = lq2, Bn1 = +gr2)
  qd2 = MatrixKick_GPUext.Quadrupole(L = lq2, Bn1 = -gr2)
  qf3 = MatrixKick_GPUext.Quadrupole(L = lq3, Bn1 = +gr3)
  qd3 = MatrixKick_GPUext.Quadrupole(L = lq3, Bn1 = -gr3)
  qf4 = MatrixKick_GPUext.Quadrupole(L = lq4, Bn1 = +gr4)
  qd4 = MatrixKick_GPUext.Quadrupole(L = lq4, Bn1 = -gr4)
  ...
  # -- beams
  # beam1.initial
  xi  = ([ 0.000,  0.000,  0.000,  0.002,  0.00200, -0.00200 ] .* ones(1024)')[:]
  pxi = ([ 0.000,  0.000,  0.000,  0.000,  0.00075, -0.00075 ] .* ones(1024)')[:]
  yi  = ([ 0.000,  0.000,  0.000,  0.001,  0.00100, -0.00100 ] .* ones(1024)')[:]
  pyi = ([ 0.000,  0.000,  0.000,  0.000,  0.00030, -0.00030 ] .* ones(1024)')[:]
  zi  = ([ 0.000,  0.000,  0.000,  0.000,  0.00000,  0.00000 ] .* ones(1024)')[:]
  pzi = ([ 0.000,  0.001, -0.001,  0.001,  0.00100,  0.00100 ] .* ones(1024)')[:]
  
  # beam1.dr1.final
  xf_dr1  = ([ 0.,  0.,                     0.,                     2.e-3,                  2.1123876489808660e-3, -2.1123876489808660e-3 ] .* ones(1024)')[:]
  pxf_dr1 = ([ 0.,  0.,                     0.,                     0.,                     7.5e-4,                -7.5e-4                ] .* ones(1024)')[:]
  yf_dr1  = ([ 0.,  0.,                     0.,                     1.e-3,                  1.0449550595923462e-3, -1.0449550595923462e-3 ] .* ones(1024)')[:]
  pyf_dr1 = ([ 0.,  0.,                     0.,                     0.,                     3.e-4,                 -3.e-4                 ] .* ones(1024)')[:]
  zf_dr1  = ([ 0.,  1.4710284465059844e-4, -1.4711135596840458e-4,  1.4710284465059844e-4,  1.4705400485512816e-4,  1.4705400485512816e-4 ] .* ones(1024)')[:]
  pzf_dr1 = ([ 0.,  1.e-3,                 -1.e-3,                  1.e-3,                  1.e-3,                  1.e-3                 ] .* ones(1024)')[:]
  ...
  #
  # 5 keV electron
  beam1_d = Bunch(species = e_minus, beta_gamma_ref = bg1,
               x = CuArray(xi), px = CuArray(pxi), y = CuArray(yi), py = CuArray(pyi), z = CuArray(zi), pz = CuArray(pzi))
  MatrixKick_GPUext.track!(beam1_d, dr1);

  beam1_h = Bunch(species = e_minus, beta_gamma_ref = bg1,
               x = Array(beam1_d.v.x), px = Array(beam1_d.v.px), y = Array(beam1_d.v.y), py = Array(beam1_d.v.py), z = Array(beam1_d.v.z), pz = Array(beam1_d.v.pz))

   @test beam1_h.v.x  ≈  xf_dr1 (rtol=5.e-13)
   @test beam1_h.v.y  ≈  yf_dr1 (rtol=5.e-13)
   @test beam1_h.v.z  ≈  zf_dr1 (rtol=5.e-13)
   @test beam1_h.v.px == pxf_dr1
   @test beam1_h.v.py == pyf_dr1
   @test beam1_h.v.pz == pzf_dr1
end
