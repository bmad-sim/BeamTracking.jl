...
macro def_integrator_struct(name)
  quote
    struct $(esc(name)) <: AbstractYoshida
      ...
      function $(esc(name))(; order::Int=4, num_steps::Int=-1, ds_step::Float64=-1.0, radiation_damping_on::Bool=false, radiation_fluctuations_on::Bool=false, ibs_num_particles::Int=0)
        _order = order
        ...
        return new(_order, _num_steps, _ds_step, radiation_damping_on, radiation_fluctuations_on, ibs_num_particles)
      end
    end
  end
end
