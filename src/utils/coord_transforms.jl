...
@inline function coord_bend_arc_transform(ds, g_ref, tilt_ref)
  @FastGTPSA begin
    ...
    trq = rot_quat((0f0, 0f0, 1f0), tilt_ref)
    r = (-ds * ang * one_cos_norm(ang), 0f0, ds * sincu(ang))
    r = quat_rotate(r, trq)
...
@inline function coord_alignment_bend_entering(x_off, y_off, z_off, x_rot, y_rot, tilt, g_ref, tilt_ref, ele_orient, L)
  ...
  r, q = coord_bend_arc_transform(0.5f0*L, g_ref, tilt_ref)
  ## println("***A: $r  :: $q")

  # Translate to coords with origin at element chord midpoint.
  ang2 = 0.5f0 * L * g_ref
  one_c = one_cos_norm(ang2)
  f = -0.25f0 * L^2 * g_ref * one_c
  r = r .+ quat_rotate((f*cos(tilt_ref), f*sin(tilt_ref), 0f0), q)
  ## println("***B: $r  :: $q")
  ...
  dq = rot_quaternion(0f0, 0f0, tilt_ref)
  q = quat_mul(q, dq)
  ## println("***X: $r  :: $q")

  # Translate from chord center to arc center.
  dr = (0.25f0 * L^2 * g_ref * one_c, 0f0, 0f0)
  r = r .+ quat_rotate(dr, q)
  ## println("***Y: $r  :: $q")

  # Transform from arc center back to entrance face.
  dr, dq = coord_bend_arc_transform(-0.5f0*L, g_ref, 0f0)
