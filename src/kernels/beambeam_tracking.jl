...
# Arguments
- `z`: Position along the beam. Positive z is at the head of the bunch.
- `crab: Crab coefficients [crab_x1, crab_x2, crab_x3, crab_x4, crab_x5]
- `crab_tilt: Crab tilt angle in radians

# Returns
- `center`: 3-element vector containing (x, y, z) position of slice in body coordinates.
...
# Arguments
- `funct`: Function to be inverted (must take a single Float64 argument)
- `y`: Value to be inverted (find x such that funct(x) = y)
- `x1`: Lower bound for search (x must satisfy x1 < x < x2)
- `x2`: Upper bound for search
- `tol`: Absolute accuracy tolerance for x

# Returns
- `x`: Solution such that funct(x) ≈ y
...
"""
    strong_beam_sigma_calc(sig_x0, sig_y0, 
                                ds,
                                beta_a_strong, alpha_a_strong,
                                beta_b_strong, alpha_b_strong,
                                beam_strength, 
...
# Arguments
- `orbit`: 6-element particle coordinate vector [x, px, y, py, z, pz]
- `x_offset`: Horizontal offset
- `y_offset`: Vertical offset  
- `z_offset`: Longitudinal offset
- `x_pitch`: Pitch angle around x-axis (radians)
- `y_pitch`: Pitch angle around y-axis (radians)
- `tilt`: Tilt/roll angle around z-axis (radians)
- `set`: true = lab to body coords, false = body to lab coords

# Returns
- `orbit_out`: Transformed 6-element coordinate vector
...
"""
function offset_particle(i, orbit, x_offset, y_offset, 
                        z_offset, x_pitch, y_pitch, 
                        tilt, set)
