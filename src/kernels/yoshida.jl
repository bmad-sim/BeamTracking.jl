...
function compute_g(::typeof(bkb_multipole!), params::P) where {P}
  g = params[6]
  w = params[7]
  costilt = w[1]
  sintilt = w[4]
...
function compute_g(::typeof(exact_curved_drift!), params::P) where {P}
  g = params[4]
  w = params[5]
  costilt = w[1]
  sintilt = w[4] 
...
function compute_g(
  ::Union{
    ...
    return compute_g(params[1], params[2])
end

compute_g(::Any, params) = (0, 0)
...
@inline function order_two_integrator!(i, coords::Coords, ker, params, photon_params, ds_step, n_steps, edge_params, ::Val{fringe_in}, ::Val{fringe_out}, L) where {fringe_in,fringe_out}
  @inbounds begin
    ...
    for step in 1:(n_steps-1)
      ...
      execute_callbacks!(i, coords, ds_step, compute_g(ker, params))
    end
    # Last step doesn't execute callbacks and does half step for radiation:
    ker(i, coords, s, params..., ds_step)
    s += ds_step
    if !isnothing(photon_params)
      stochastic_radiation!(i, coords, s, photon_params..., ds_step / 2)
