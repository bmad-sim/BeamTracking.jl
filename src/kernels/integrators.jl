...
@inline function order_four_integrator!(i, coords::Coords, ker, params, photon_params, ds_step, n_steps, edge_params, ::Val{fringe_in}, ::Val{fringe_out}, ::Val{use_optimized_schemes}, L) where {fringe_in, fringe_out, use_optimized_schemes}
  @inbounds begin
    if use_optimized_schemes
      w0 = -0.6579630871775028477799196480191312730312347412109375*ds_step
      w1 =  0.414490771794375711944979912004782818257808685302734375*ds_step
    else
...
@inline function order_six_integrator!(i, coords::Coords, ker, params, photon_params, ds_step, n_steps, edge_params, ::Val{fringe_in}, ::Val{fringe_out}, ::Val{use_optimized_schemes}, L) where {fringe_in, fringe_out, use_optimized_schemes}
  @inbounds begin
    if use_optimized_schemes
      w0 =  0.39907751534871587459988795520665*ds_step
      w1 =  0.11805530653002387170273438954049*ds_step
      w2 = -0.35000324893920896516170830911323*ds_step
      w3 =  0.12961893756907034772505366537091*ds_step
      w4 =  0.13070531011449225190542755785015*ds_step
      w5 =  0.13346562851074760407046858832209*ds_step
      w6 =  0.13861930854051695245808013042625*ds_step
    else
...
@inline function order_eight_integrator!(i, coords::Coords, ker, params, photon_params, ds_step, n_steps, edge_params, ::Val{fringe_in}, ::Val{fringe_out}, ::Val{use_optimized_schemes}, L) where {fringe_in, fringe_out, use_optimized_schemes}
  @inbounds begin
    if use_optimized_schemes
      w0 = -0.38263596012643665350944670744040*ds_step
      w1 =  0.11699135019217642180722881433533*ds_step
      w2 =  0.12581718736176041804392391641587*ds_step
      w3 =  0.12603912321825988140305670268365*ds_step
      w4 =  0.11892905625000350062692972283951*ds_step
      w5 =  0.11317848435755633314700952515599*ds_step
      w6 = -0.24445266791528841269462171413216*ds_step
      w7 = -0.23341414023165082198780281128319*ds_step
      w8 =  0.35337821052654342419534541324080*ds_step
      w9 =  0.10837408645835726397433410591546*ds_step
      w10 = 0.10647728984550031823931967854896*ds_step
    else
