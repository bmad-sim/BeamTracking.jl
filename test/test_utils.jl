...
const high_order = true
const N = high_order ? 10 : 3

const D1 = Descriptor(6, 1)   # 6 variables 1st order
const DN = Descriptor(6, N)   # 6 variables Nth order
...
function coeffs_approx_equal(v_expected, v_calculated, ϵ)
  n = GTPSA.numcoefs(v_calculated[1])
  all_ok = true
  for i in 1:length(v_calculated)
      for j in 0:n-1
...
function quaternion_coeffs_approx_equal(q_expected, q_calculated, ϵ)
  ...
  n = GTPSA.numcoefs(q_calculated.q0)
  all_ok = true
