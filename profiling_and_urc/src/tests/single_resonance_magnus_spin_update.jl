...
@inline function S_dynamics!(Y_n1, Y_n2, RS, dt, nu0, sigma0)
    ...
    x[1] = 0.5 * (((dt / 2) * (Y_n1[1] + Y_n2[1])) - (dt^2 * 0.14433756729740643 * (nu0 * sigma0 * (Y_n1[2] - Y_n2[2])))) # origionally b11 + b21
    x[2] = 0.5 * (((dt / 2) * (Y_n1[2] + Y_n2[2])) - (dt^2 * 0.14433756729740643 * (nu0 * sigma0 * (Y_n2[1] - Y_n1[1])))) # origionally b12 + b22
    x[3] = 0.5 * (((dt / 2) * (2 * nu0)) - (dt^2 * 0.14433756729740643 * (sigma0 * (Y_n1[1] * Y_n2[2] - Y_n2[1] * Y_n1[2]))))
...
@inline function magnus_step!(Y, S, dt, beta, nu0, epsilon, sigma0)
    """
...
@inline function magnus_solve!(Y, S, steps, dt, beta, nu0, epsilon, sigma0)
    """
    Solves the particle dynamics from time t0 to tf with initial conditions Y0 and S0.
    """
    for _ = 1:steps
        magnus_step!(Y, S, dt, beta, nu0, epsilon, sigma0)
    end
    return
end

function single_resonance_track_magnus(Y, S, nu0, sigma0, beta, epsilon, steps, dt)
    ...
    thread_S = MVector{4, Float64}(S[thread, 1], S[thread, 2], S[thread, 3], S[thread, 4])

    magnus_solve!(thread_Y, thread_S, steps, dt, beta, nu0, epsilon, sigma0)

    Y[thread, 1] = thread_Y[1]
    Y[thread, 2] = thread_Y[2]

    S[thread, 1] = thread_S[1]
    S[thread, 2] = thread_S[2]
    S[thread, 3] = thread_S[3]
    S[thread, 4] = thread_S[4]
    end
    end
    return
end
...
function single_resonance_track_magnus_threads(Y, S, nu0, sigma0, beta, epsilon, steps, dt)
