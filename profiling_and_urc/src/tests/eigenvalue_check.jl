using Plots, DifferentialEquations, LinearAlgebra, StaticArrays, Statistics, Distributions, SpecialFunctions, DelimitedFiles, CUDA
...
all_eigen = zeros(Complex, measurements, n_particles, 3)
...
for i = 1:measurements
    ...
    if (i > 1)
        ...
        for j = 1:n_particles
            ...
            spins[i, j, :] = R * spins[i-1, j, :]
            spin_sum += norm(spins[i, j, :])
        end
    end

    # all_polarizations[i] = norm(spin_sum ./ n_particles)
    all_polarizations[i] = spin_sum / n_particles
    println(all_polarizations[i])
end
all_polarizations[1] = 1.0

anim1 = @animate for i = 1:measurements
    θ = range(0, 2π, length=500)
    x = cos.(θ)
    y = sin.(θ)


    plot(x, y, aspect_ratio=:equal, legend=false, title="Particle Rotation Eigenvalues")
    for j in n_particles
        for k in 1:3
            if abs(all_eigen[i, j, k]) > 1
                scatter!([real(all_eigen[i, j, k])], [imag(all_eigen[i, j, k])], color=:red)
            elseif abs(all_eigen[i, j, k]) < 1
                scatter!([real(all_eigen[i, j, k])], [imag(all_eigen[i, j, k])], color=:blue)
            else
                scatter!([real(all_eigen[i, j, k])], [imag(all_eigen[i, j, k])])
            end
        end
    end
end

gif(anim1, "eigen_circle_check.gif", fps=15)

anim2 = @animate for i = 1:measurements
    eigen_vec = vec(all_eigen[i, :, :])
    histogram([abs.(abs.(eigen_vec) .- 1)])
end

gif(anim2, "eigen_hist_check.gif", fps=15)