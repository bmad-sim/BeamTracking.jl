...
@inline function step!(Y, R, dt, beta, nu0, epsilon, sigma0)
    ...
    Y_n1[1] = Y[1]
    Y_n1[2] = Y[2]
    step_beta = 62.45 + (nu_0 / 30) * randn()
    step_orbit!(Y_n1, Y, dt, step_beta, epsilon)


    return
end

@inline function magnus_solve!(Y, R, steps, dt, beta, nu0, epsilon, sigma0)
    """
...
function single_resonance_track_magnus_6_jim(Y, R, nu0, sigma0, beta, epsilon, steps, dt)
    """
