...
@inline function magnus_solve!(Y, R, steps, dt, beta, nu0, epsilon, sigma0, delta)
    """
    Solves the particle dynamics from time t0 to tf with initial conditions Y0 and S0.
    """
    true_nu0 = nu0
    for _ = 1:steps
        nu0 = true_nu0 * (1 + (randn() * delta * sqrt(dt)))
        step!(Y, R, dt, beta, nu0, epsilon, sigma0)
    end
    return
end

function single_resonance_track_magnus_6_nu_noise(Y, R, nu0, sigma0, beta, epsilon, delta, steps, dt)
    """
    ...
    if thread <= size(Y)[1]
        @inbounds begin
            ...
            magnus_solve!(thread_Y, thread_R, steps, dt, beta, nu0, epsilon, sigma0, delta)
...
function single_resonance_track_magnus_6_threads(Y, R, nu0, sigma0, beta, epsilon, steps, dt)
    """
    Function to track the spin of a particle using a single resonsance model and a Magnus expansion solver.
    """
    n = size(Y, 1)
