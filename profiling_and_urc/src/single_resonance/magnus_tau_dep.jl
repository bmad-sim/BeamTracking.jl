...
# plot!(t, exp.(-10e-11 * t))
maxima = findmaxima(all_polarizations)
# scatter!(t[maxima.indices], all_polarizations[maxima.indices])

t = collect(t)
log_polarizations = log.(all_polarizations[1:end])
log_peaks = log.(all_polarizations[maxima.indices])

# A1 = [zeros(measurements) t[1:end]]
# log_fit = A1 \ log_polarizations
# plot!(t[1:end], exp.(log_fit[2] * t[1:end]), label="log_fit")

# A2 = [zeros(length(maxima.indices)) t[maxima.indices]]
# log_peak_fit = A2 \ log_peaks
# plot!(t[maxima.indices], exp.(log_peak_fit[2] * t[maxima.indices]), label="log_fit")
...
# plot(t[1:end], exp.(-log_fit[2] * t[1:end]), label="Data")
# plot!(t[1:end], exp.(-a * t[1:end]), label="Exact solve")

# model1(t, param) =  exp.(param[1] * t)
# param1 = [-0.001]
# lsq_fit = curve_fit(model1, t, all_polarizations, param1)
# plot!(t, exp.(lsq_fit.param[1] * t), label="Least Squares log_fit Two Param", yscale=:log10)
# println("Tau_dep: ", lsq_fit.param)

# cof = 1 / (619 * 2π)
# τ_dep_inv(ζ; cof = 1) = cof * ((ζ^2 - 1) * -expintx(ζ^2) + 1)
# a = τ_dep_inv((nu0 - beta) / sigma0; cof = cof)
# println("\nDirect: ", -a)
# println("Magnus solve: ", lsq_fit.param[1])
# relative_error = abs(-a - lsq_fit.param[1]) / a
# println("Relative error: ", relative_error)

# model2(t, param) = exp.(param[1] * t) 
# param2 = [-0.001]
# lsq_fit = curve_fit(model2, t, all_polarizations, param2)
# plot!(t / 2pi, exp.(lsq_fit.param[1] * t), label="Least Squares log_fit One Param")
# println("Tau_dep: ", lsq_fit.param)
# savefig("nu_scan_base/polarizations_$dt" * "_$orbits" * "_$n_particles" * "_$nu0" * "_ISF.png")
writedlm("polarizations_$dt" * "_$orbits" * "_$n_particles" * "_$nu0" * "_up.txt", all_polarizations)